                ...
                case parser::Kind::Bool:
                    return builder::Result(
                        ...
                        llvm::ConstantInt::get(
                            llvm::Type::getInt1Ty(builder.context),
                            node->as<parser::Bool>()->value),
                        ...
                        if (current) {
                            ...
                            auto variable = new llvm::GlobalVariable(
                                *builder.module, initial->getType(),
                                true, llvm::GlobalVariable::LinkageTypes::PrivateLinkage,
                    ...
                    case parser::Kind::Array: {
                        ...
                        utils::Typename subType = results.empty()
                            ? utils::PrimitiveTypename::from(utils::PrimitiveType::Any)
                            : results.front().type;

                        if (!std::all_of(results.begin(), results.end(), [&subType](const builder::Result &result) {
                        ...
                        if (current) {
                            ...
                            for (size_t a = 0; a < results.size(); a++) {
                                ...
                                llvm::Value *point = current->CreateInBoundsGEP(value, {
                                    llvm::ConstantInt::get(llvm::Type::getInt64Ty(builder.context), 0),
                                    index
                                });
    ...
    builder::Wrapped Scope::makeExpressionNounModifier(const hermes::Node *node, const builder::Wrapped &result) {
        switch (node->is<parser::Kind>()) {
            ...
            case parser::Kind::Index: {
                ...
                std::optional<builder::Result> indexConverted = convert(index, utils::PrimitiveTypename {
                    utils::PrimitiveType::ULong
                });
                ...
                auto indexArray = [&]() -> llvm::Value * {
                    ...
                    switch (arrayType->kind) {
                        ...
                        case utils::ArrayKind::UnboundedSized: // TODO: no good for stack allocated arrays
            ...
            case parser::Kind::Unary:
                switch (operation.op->as<parser::Unary>()->op) {
                    case utils::UnaryOperation::Not: {
                        std::optional<builder::Result> converted = convert(value, utils::PrimitiveTypename {
                            utils::PrimitiveType::Bool
                        });
                    ...
                    case utils::UnaryOperation::Reference:
                        ...
                        return builder::Result(
                            builder::Result::FlagTemporary,
                            value.value,
                            utils::ReferenceTypename { std::make_shared<utils::Typename>(value.type) },
                            &statementContext
                            );
                    ...
                    case utils::UnaryOperation::Fetch: {
                        if (auto refType = std::get_if<utils::ReferenceTypename>(&value.type)) {
                            return builder::Result(
                                ...
                                get(value),
                                *std::get<utils::ReferenceTypename>(value.type).value,
                                &statementContext
                                );
                        } else {
                ...
                case parser::Kind::Ternary: {
                    std::optional<builder::Result> inferConverted = convert(value, utils::PrimitiveTypename {
                        utils::PrimitiveType::Bool
                    });
                    ...
                    builder::Scope trueScope(
                        operation.op->children[0]->as<parser::Expression>(),
                        *this, current.has_value());
                    builder::Scope falseScope(
                        operation.op->children[1]->as<parser::Expression>(),
                        *this, current.has_value());
